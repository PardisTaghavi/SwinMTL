...
class SwinTransformerBlockPre(nn.Module):
    ...
    Args:
        dim (int): Number of input channels.
        num_heads (int): Number of attention heads.backbone
        window_size (int): Window size.
        shift_size (int): Shift size for SW-MSA.
        mlp_ratio (float): Ratio of mlp hidden dim to embedding dim.
        qkv_bias (bool, optional): If True, add a learnable bias to query, key, value. Default: True
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=3, in_chans=4, embed_dim=96, norm_layer=None): ############################################333
        super().__init__()
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size) #in_chans changed o 4
        if norm_layer is not None:
